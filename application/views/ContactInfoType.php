...
<script>
	$().ready(function() {
		...
		$("#signupForm").validate({
		   rules: {
			...
			messages: {
               ...
               {
                    $.ajax({
                            url: "<?php echo $base; ?>/index.php/AddContactInfoTypeController/AddContactInfoType",
     ...
     function updatepartytype(){
                    $.ajax({
                    url:"<?php echo $base; ?>/index.php/AddContactInfoTypeController/updateContactInfoType",    
                     data: GetFormValues('signupForm'),
                    type: "POST",
                    success: function(data){
                    var myobj;
    ...
    $.ajax({
        url: "<?php echo $base; ?>/index.php/AddContactInfoTypeController/getContactInfoType",
        type:'POST',
        //data:{data},
        success: function(data) {
           for (var i = 0; i < JSON.parse(data).length; i++) {
                   TableData += '<tr><td>' + JSON.parse(data)[i].Key + '</td><td>' + JSON.parse(data)[i].Description + '</td><td><a href="#" onclick="editContactInfoType('+JSON.parse(data)[i].Id+'); return false;">Edit</a> <a href="" onclick="deleteContactInfoType('+JSON.parse(data)[i].Id+'); return false;" > Delete</a></td></tr>';
        }
...
function editContactInfoType(id){
                 $('#signupForm').show();
                    $.ajax({
                    url:"<?php echo $base; ?>/index.php/AddContactInfoTypeController/editContactInfoType",    
                     data: {id: id},
                    type: "POST",
                    success: function(data){
                    var myobj;
...
function deleteContactInfoType(id)
{
      var result = confirm("Want to delete?");
        if (result) {
           $.ajax({
          type: "GET",
          url: "<?php echo $base;?>/index.php/AddContactInfoTypeController/deleteContactInfoType/"+id,
